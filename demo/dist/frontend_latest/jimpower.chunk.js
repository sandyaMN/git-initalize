...
const demoEntitiesJimpower = () => Object(_src_fake_data_entity__WEBPACK_IMPORTED_MODULE_0__["convertEntities"])({
  "zone.powertec": {
...
class CardModder extends lit_element__WEBPACK_IMPORTED_MODULE_0__["LitElement"] {
  setConfig(config) {
  ...
  set hass(hass) {
    ...
    if (this.templated) this.templated.forEach(k => {
...
if (!window.cardTools) {
  ...
  cardTools.createThing = (thing, config) => {
    ...
    if (!config || typeof config !== "object" || !config.type) return _createError(`No ${thing} type configured`, config);
    let tag = config.type;
    ...
    if (tag.startsWith(CUSTOM_TYPE_PREFIX)) tag = tag.substr(CUSTOM_TYPE_PREFIX.length);else tag = `hui-${tag}-${thing}`;
    if (customElements.get(tag)) return _createThing(tag, config); // If element doesn't exist (yet) create an error

    const element = _createError(`Custom element doesn't exist: ${tag}.`, config);
